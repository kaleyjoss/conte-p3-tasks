 ...
 *
 * last modified by Jamie Chiu - Jan 18, 2022 to run as a practice trial
 */

jsPsych.plugins['practice'] = (function() {
	...
	plugin.info = {
		...
		parameters: {
			ball_choice: {
				...
				pretty_name: 'balloon to choose',
				default: 'left',
				array: true,
				description: 'which balloon subject is to practice with',
			},
			npl:  {
	...
	plugin.trial = function(display_element, trial) {
		...
		var kpl = 0;
		var kpr = 0;
		var instructions = []; // which balloon subject is to choose
		var ball_choice = [];
		...
		var after_response = function(info) {
			...
			if      (firstkey.length==0 & (info.key == key_l | info.key == key_r)) {
				...
				if      (firstkey == key_l) {
					kpl = kpl + 1; rtx = rtx + ', ' + rtn;
				}  // so do strings instead
				else if (firstkey == key_r) {
					kpr = kpr + 1; rtx = rtx + ', ' + rtn;
				}
			}
		...
		function feedback(kpr,kpl){
			html = '<p style="color:green; text-align: center; font-size:200%; line-height: 200%">Well done!</p>';
			display_element.innerHTML = '<div id="jspsych-reconstruction-stim-container">'+html+'</div>';
		};


		function draw(kpr,kpl) {
			instructions = 'press d to choose the LEFT balloon<br>'+
			'press f to choose the RIGHT balloon<br>'+
			'press j to pump up the balloon.<br>'
		...
		function endTrial() {
			...
			// save data
			var trial_data = { // TODO: add P to denote practice trial
				"trial_name": 'practice trial',
				"practice_total_trial_time": total_trial_time,
				"practice_trial_start_time": startTime,
				"practice_chosen_balloon": firstkey,
				//"chosen_reward": rewchosen,
				"practice_chosen_effort": effchosen,
				//"unchosen_reward": rewnotchosen,
				//"unchosen_effort": effnotchosen,
				"practice_bp_times": rtx,
				"practice_key_press": firstkey,
				"practice_rt": rtx,
				"practice_stimulus": '{ "Left effort": '+trial.npl+', "Right effort": '+trial.npr+'}',
				"practice_nonresponses": nonresp,
				"practice_escaped": escaped,
				"practice_timeout": timeout,
			};
	...
	// 					display_element.innerHTML = '<div id="jspsych-reconstruction-stim-container"><p style="color:red;   text-align: center; font-size:200%; line-height: 200%">Please Remember: </p><ul><li>d &nbsp; to CHOOSE the LEFT balloon.</li><li>f &nbsp; to CHOOSE the RIGHT ballooon.</li><li>j &nbsp; to PUMP UP the balloon.</li></ul></div>';
	// 					endTrial();
	// 			 	}
	// 		 	}, trial.dur_decision);
	// 	}
	...
	//    jsPsych.pluginAPI.setTimeout(function() {
	// 		choices =  jsPsych.data.get().select('practice_chosen_balloon').values;
	// 		nonresp = choices.filter(x=> x=="NaN").length + 1
